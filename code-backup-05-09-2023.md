...
class NextPatientBirthdayIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        try:
            # Query the database to find all birthdays and sort them by date_of_birth
            ...
            if upcoming_birthdays.count() > 0:
                # Generate a list of upcoming birthdays
                birthday_list = []
                for birthday in upcoming_birthdays:
                    first_name = birthday.get("first_name")
                    date_of_birth = birthday.get("date_of_birth")
                    birthday_list.append(f"{first_name}'s birthday on {date_of_birth}")

                # Combine the list of birthdays into a single string
                birthdays_text = ", ".join(birthday_list)
                speak_output = f"The upcoming birthdays are: {birthdays_text}."
            else:
