...
#Update Patient Info Intent
class UpdatePatientInfoIntentHandler(AbstractRequestHandler):

    def can_handle(self, handler_input):
        return is_intent_name("UpdatePatientInfoIntent")(handler_input)

    def handle(self, handler_input):
        try:
            # Retrieve the patient's name from the slots
            patient_name = handler_input.request_envelope.request.intent.slots["UpdatePatientNameSlot"].value

            # Check if the patient exists in the database
            existing_patient = appointments_collection.find_one({"first_name": patient_name})

            if existing_patient:
                # Ask the user what information they want to update
                speak_output = f"Sure, {patient_name}. What information would you like to update? You can say something like 'Update my name' or 'Change my date of birth.'"

                # Store the patient's name in session attributes for future reference
                handler_input.attributes_manager.session_attributes["patient_name"] = patient_name

                # Set a session attribute to indicate that an update is in progress
                handler_input.attributes_manager.session_attributes["update_in_progress"] = True
            else:
                speak_output = f"Sorry, I couldn't find a patient with the name {patient_name} in the database. Please specify a valid name."

        except Exception as e:
            logger.error("Error updating data in MongoDB: %s", str(e))
            speak_output = "Sorry, there was an issue updating the patient's information. Please try again later."
...
#Specify Date Of Birth Update Intent - works with above intent to update the details of the date of birth.
class SpecifyDateOfBirthUpdateIntentHandler(AbstractRequestHandler):

    def can_handle(self, handler_input):
        return is_intent_name("SpecifyDateOfBirthUpdateIntent")(handler_input)

    def handle(self, handler_input):
        try:
            # Handle the date of birth update here
            day = handler_input.request_envelope.request.intent.slots["day"].value
            month = handler_input.request_envelope.request.intent.slots["month"].value
            year = handler_input.request_envelope.request.intent.slots["year"].value
            ...
            if patient_name:
                # Update the date of birth in MongoDB for the specified patient
                appointments_collection.update_one({"first_name": patient_name}, {"$set": {"date_of_birth": date_of_birth}})
                
                speak_output = f"Thank you, {patient_name}. Your date of birth has been updated to {date_of_birth}."
            else:
                speak_output = "Sorry, I couldn't find the patient's name in the session. Please specify the patient's name first."

        except Exception as e:
            logger.error("Error updating date of birth: %s", str(e))
            speak_output = "Sorry, there was an issue updating the date of birth. Please try again later."
        ...
        try:
            ...
            if all_patients_data:
                ...
                for patient_data in all_patients_data:
                    ...
                    if date_of_birth:
                        date_parts = date_of_birth.split("_")
                        if len(date_parts) == 3:
                            birth_day = int(date_parts[0])
...
sb.add_request_handler(UpdatePatientInfoIntentHandler())
sb.add_request_handler(SpecifyDateOfBirthUpdateIntent())
