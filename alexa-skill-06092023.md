![image](https://github.com/expertcloudconsultant/alexa-skill/assets/69172523/0b8f2e9c-75a3-4f8a-8d24-a34d07faaef3)
...
class ListAllPatientsIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        try:
            # Query the database to retrieve all patient names
            all_patients = appointments_collection.distinct("first_name")

            if all_patients:
                patients_list = ", ".join(all_patients)
...
class NextPatientBirthdayIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        try:
            ...
            # Query the database to find all birthdays
            upcoming_birthdays = appointments_collection.find(
            ...
            for birthday in upcoming_birthdays:
                date_of_birth = datetime.strptime(birthday.get("date_of_birth"), "%d-%m-%Y")
            ...
            if next_birthday:
                ...
                speak_output = f"The next birthday is {first_name}'s birthday on {date_of_birth}."
            else:
                speak_output = "There are no upcoming birthdays in the database."

        except Exception as e:
            logger.error("Error querying the database: %s", str(e))
            speak_output = "Sorry, there was an issue retrieving the upcoming birthdays. Please try again later."
